...
**Server (hosted):** `https://mcp.petbrains.dev/mcp` (HTTP, read-only)
...
- Not in tool list — fall back to `scripts/search.py` with equivalent CSV queries and tell the user: *"designlib MCP not connected; using local CSV fallback. Install: `claude mcp add --transport http designlib https://mcp.petbrains.dev/mcp`"*
